...
import 'package:carpool_project/SuccessPage.dart';
import 'package:flutterflow_ui/flutterflow_ui.dart';
import 'package:firebase_auth/firebase_auth.dart';

import 'HomePage.dart';

class RidePageUni extends StatefulWidget {
  const RidePageUni({Key? key}) : super(key: key);

  @override
  _RidePageUniState createState() => _RidePageUniState();
}

class _RidePageUniState extends State<RidePageUni> with TickerProviderStateMixin {
  late RidePageUniModel _model;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: StreamBuilder<QuerySnapshot>(
        stream: FirebaseFirestore.instance.collection('trips').where('time', isEqualTo: '7:30 AM').snapshots(),
...
class RidePageUniModel extends ChangeNotifier {
  final unfocusNode = FocusNode();

  bool _isTimeConstraintActive = true; // Default is active, change as needed
  ...
  bool isBookingAllowed(String tripDate) {
    try {
      ...
      // Attempt to parse the trip date using the first format (dd-MM-yyyy)
      ...
      try {
        ...
        // If parsing fails, attempt to parse using the second format (dd/MM/yyyy)
        List<String> dateParts = tripDate.split('/');
        int day = int.parse(dateParts[0]);
        int month = int.parse(dateParts[1]);
        int year = int.parse(dateParts[2]);
      ...
      // Check if the trip date is not today
      bool isNotToday =
          parsedTripDate.day != now.day || parsedTripDate.month != now.month || parsedTripDate.year != now.year;

      return !isTimeConstraintActive || (isNotToday || (now.isAfter(allowedStartTime) && now.isBefore(allowedEndTime)));
    } catch (e) {
      print('Error parsing date: $e');
      return false; // Return false if there is an error in parsing the date
    }
  }
...
RidePageUniModel createModel() => RidePageUniModel();
